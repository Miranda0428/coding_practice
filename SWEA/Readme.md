...
![image](https://user-images.githubusercontent.com/17174701/145945987-5328507f-15c3-4f23-809b-a4306655409d.png)

  
  추가로
  

  ![image](https://user-images.githubusercontent.com/17174701/145946079-fc56e35c-1c04-4e49-8528-c31c20ae07df.png)
...
그 원인을 찾아보아야 한다.
  ...
  **변수의 초기화(간담회 참석 등)**
  ...
  테스트 케이스에서 다음 테스트 케이스롤 넘어갈때 변수를 초기화 해주지 않아 계속 오답이 나왔다.
  
  vector 전역변수의 경우 push_back을 쓰면 테스트케이스 마다 계속해서 뒤로 입력값이 붙는다.
  
  
 ** 다익스트라 최적화(간담회 참석)**

  다익스트라는 우선순위 큐를 써서 최적화를 하되, pair를 큐에 넣으면 첫번째 원소를 기준으로 큰 값이 먼저 나오게 된다.
  
  따라서 첫번째 원소로 distance를 넣어야 하고 넣을 때와 뺄 떄 - 를 붙여줘야 작은 값을 먼저 나오게 할 수 있다.
  ...
  ** 위상정렬 (줄세우기)**
  ...
  단순 위상정렬은 dfs를 이용하면 된다. 원래 dfs에서 result.push_back 만 추가되었음을 확인하자.
  
  iterator를 이용한 vector 데이터의 for 문 활용법이 유용하다.
  
  reverse 함수를 이용한 for문 역방향 구현도 유용하다.
  
  
  **오일러 파이 함수(파이의 합)**
  
  오일러 파이 함수 계산시 Φ(30) = 30(1 - 1/2)(1 - 1/3)(1 - 1/5) = 30((2-1)/2)((3-1)/3)((5-1)-1/5) 으로 구현
  
  시간초과로 통과하지 못한 상태(같은 코드인데 JAVA 코드는 Pass이고 C++ 코드는 시간초과 란다.)
